...
During a rollback, multiple blocks may be invalidated and replaced simultaneously. This can result in a flurry of events being dispatched to your webhook endpoint in an unpredictable order. Developers should account for this behavior by implementing logic to handle out-of-order delivery of block events.
...
However, it's worth noting that if any of the retry attempts succeed, the operation is considered successful, and only one failed request will be logged if all attempts ultimately fail. 
:::
