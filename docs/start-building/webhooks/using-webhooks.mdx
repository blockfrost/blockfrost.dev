...
Here is a quick example of web application that implements `/webhook` endpoint and leverages Blockfrost SDK to verify the webhook signature:

:::info Missing an example for your favorite language?
Earn a bounty for contributing an example in your favorite language. Everyone can participate earn a reward for contributions. [Join the Blockfrost Bounty Program now!](https://github.com/blockfrost/blockfrost.dev/issues?q=is%3Aissue+is%3Aopen+label%3ABounty%21)
:::
      ...
      case "delegation":
        ...
        for (const transaction of payload) {
          for (const delegation of transaction.delegations) {
            console.log(`Delegation from address ${delegation.address} inclued in tx ${transaction.tx.hash}`);
          }
        }
        break;
...
In the Webhook settings, set the webhook's endpoint to the URL you will see in the output (if you are using [the example](using-webhooks#process-a-webhook-request) do not forget to append `/webhook` at the end).
